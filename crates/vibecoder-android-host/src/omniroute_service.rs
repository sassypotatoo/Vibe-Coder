...
use vibecoder_gateway_contract::{GatewayCredential, GatewayExecutionProfile, ModelGateway};
...
async fn wait_for_omniroute_readiness(
    ...
    let mut last_profile: Option<GatewayExecutionProfile> = None;
        ...
        match client.execution_profile(GatewayCredential::Anonymous).await {
            Ok(profile) => {
                ...
                if consecutive >= READY_CONSECUTIVE_ATTESTATIONS {
                    ...
                    let profile = last_profile.take().ok_or_else(|| {
                        host_error("android_host_omniroute_readiness_profile_missing")
                    })?;
