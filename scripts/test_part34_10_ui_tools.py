...
assert 'base_alpha_apk_with_direct_node_setup_download_not_device_execution' in alpha_evidence
...
assert 'Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES' in delivery
assert 'vibecoder-node-runtime-24.19.0-v31' in delivery
assert 'com.google.android.play:feature-delivery' not in base_gradle
assert 'SplitCompatApplication' not in base_manifest
assert '<dist:on-demand' not in feature
assert '<dist:install-time>' in feature
print('Part 34.10.15 UI/runtime/direct-node-setup pre-compile regression PASSED')
